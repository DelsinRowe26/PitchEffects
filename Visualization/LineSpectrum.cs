...
namespace WinformsVisualization.Visualization
{
    public class LineSpectrum : SpectrumBase
    {
        ...
        public LineSpectrum(FftSize fftSize)//Линейный спектр
        {
            FftSize = fftSize;
        }
        ...
        public Bitmap CreateSpectrumLine(Size size, Brush brush, Color background, bool highQuality)//Создать спектральную линию
        {
        ...
        public Bitmap CreateSpectrumLine(Size size, Color color1, Color color2, Color background, bool highQuality)//Создать спектральную линию
        {
            if (!UpdateFrequencyMappingIfNessesary(size))
                return null;
        ...
        private void CreateSpectrumLineInternal(Graphics graphics, Pen pen, float[] fftBuffer, Size size)//Создать внутреннюю линию спектра
        {
        ...
        protected override void UpdateFrequencyMapping()//Обновление карт частот
        {
            _barWidth = Math.Max(((_currentSize.Width - (BarSpacing * (BarCount + 1))) / BarCount), 0.00001);
            base.UpdateFrequencyMapping();
        }

        private bool UpdateFrequencyMappingIfNessesary(Size newSize)//Обновите сопоставление частот, если необходимо
        {
        ...
        private void PrepareGraphics(Graphics graphics, bool highQuality)//Подготовить графику
        {
            if (highQuality)
            {
